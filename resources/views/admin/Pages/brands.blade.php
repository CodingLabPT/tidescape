                ...
                <div class="col-lg-12">
                    <div class="breadcrumb-contents">
                        ...
                        <ul class="breadcrumb-contents-list list-style-none">
                            ...
                            <li class="breadcrumb-contents-list-item"> {!! __('backend/Pages/brands.title') !!} </li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="container">
            <div class="dashboard-contents-wrapper">
                ...
                <div class="dashboard-right-contents mt-4 mt-lg-0" style="border-radius:5px">
                    <div class="breadcrumb-contents">
                        <p>{!! __('backend/Pages/brands.title') !!}</p>
                        <a href="{{ route('brands.create') }}"><i title="{{ __('backend/Pages/brands.add') }}" style="font-size: 30px" class="las la-plus-circle"></i></a>
                    </div>
                    <br><br>
                    @if (count($brands) == 0)
                        <div class="alert alert-danger" role="alert">
                            {!! __('backend/Pages/brands.no_results') !!}
                        </div>
                    @else
                    <div class="table-responsive">
                        <table id="example" class="table table-striped" style="width:100%">
                            <thead>
                                <tr>
                                    <tr>
                                        <th>{{ __('backend/Pages/brands.name') }}</th>
                                        <th>{{ __('backend/Pages/brands.url') }}</th>
                                        <th style="text-align: right">{{ __('backend/Pages/brands.acoes') }}</th>
                                    </tr>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($brands as $brand)
                                    <tr>
                                        <td>{{ $brand->name }}</td>
                                        <td style="width: 100px"><a target="_blank" href="{{ $brand->url }}"><img src="{{ asset($brand->logo) }}"></a></td>
                                        <td style="text-align:right">
                                            {{--  <a title="{{ __('backend/Pages/brands.editar') }}" class="btn btn-warning btn-sm" href="{{ route('brands.edit', $brand->id) }}"> <i class="fas fa-info-circle"></i>{{ __('backend/Pages/messages.details') }}</a> --}}
                                            <a title="{{ __('backend/Pages/brands.apagar') }}" class="btn btn-danger btn-sm" onclick="return confirm('{{ __('backend/Pages/brands.sure') }}')" href="{{ route('brands.destroy', $brand->id) }}"> <i class="fas fa-trash-alt"></i>{{ __('backend/Pages/brands.apagar') }}</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
