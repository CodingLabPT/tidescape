    ...
    <section class="login-area py-10 bg-gray-100">
        <div class="container mx-auto flex justify-center items-center">
            <div class="bg-white shadow-md rounded-lg overflow-hidden flex max-w-4xl">
                ...
                <!-- Form Section -->
                <form method="POST" action="{{ route('login.store') }}" class="w-full md:w-1/2 p-8">
                    @csrf
                    <div class="mb-4">
                    ...
                    <br>
                    <div>
                        <x-text-input id="login" class="block mt-1 w-full" type="hidden" name="tour" value="{{ $id }}" required autofocus autocomplete="username" />
                    </div>
                    ...
                    <p class="mt-4 text-center text-gray-600">{{ __('auth.login.no_account') }} <a href="{{ route('register.index', ['id' => $id]) }}" class="text-blue-500 hover:text-blue-800">{{ __('auth.login.register_now') }}</a></p>
