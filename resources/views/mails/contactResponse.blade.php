                        ...
                        <td style="box-sizing: border-box; padding: 20px;">
                            <table border="0" cellpadding="0" cellspacing="0" style="width: 100%">
                                ...
                                <tr>
                                    <td>
                                        @if ($locale == 'es')
                                            <h1>Respuesta al mensaje de contacto</h1>
                                            <p><strong>Mensaje enviado: </strong>{{ $msg }}</p>
                                            <small>Enviada el dè´øa {{ $msg_enviada_em }} </small>
                                            <hr>
                                            <p><strong>Respuesta obtenida: </strong>{{ $resp }}</p>
                                            <small>Respondida el dè´øa {{ $resp_obtida_em }}</small>
                                        @elseif($locale == 'en')
                                            <h1>Contact Message Reply</h1>
                                            <p><strong>Sent Message: </strong>{{ $msg }}</p>
                                            <small>Sent on  {{ $msg_enviada_em }} </small>
                                            <hr>
                                            <p><strong>Received Reply: </strong>{{ $resp }}</p>
                                            <small>Replied on {{ $resp_obtida_em }}</small>
                                        @else
                                            <h1>Resposta a mensagem de Contato</h1>
                                            <p><strong>Mensagem enviada: </strong>{{ $msg }}</p>
                                            <small>Enviada a {{ $msg_enviada_em }} </small>
                                            <hr>
                                            <p><strong>Resposta obtida: </strong>{{ $resp }}</p>
                                            <small>Respondida em {{ $resp_obtida_em }}</small>
